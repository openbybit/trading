...
// New creates a new Consistent object with a default setting of 50 replicas for each entry.
//
...
// Diff show if elements passed in are different between those conhash have
func (c *Consistent) Diff(elts []string) bool {
	c.RLock()
	defer c.RUnlock()

	if len(c.members) != len(elts) {
		return true
	}
	for _, e := range elts {
...
func (c *Consistent) Set(elts []string) {
	...
	for _, v := range elts {
		...
		diff = true
		for i := 0; i < c.NumberOfReplicas; i++ {
			c.circle[c.hashKey(c.eltKey(v, i))] = v
		}
		c.members[v] = true
		c.count++
	}
